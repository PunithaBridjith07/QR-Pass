...
export function isArtistAvailable(http: HttpClient, controlName1: string, controlName2: string, editEventId?: string): AsyncValidatorFn {
    return (control: AbstractControl): Observable<ValidationErrors | null> => {
        ...
        const date = form?.get(controlName2)?.value;
        const URL = `${baseURL}/_design/Views/_view/eventbyid`;

        return http.get<{ rows: any[] }>(URL, { headers: headers }).pipe(
            switchMap((response) => {
                const updateEvent = response.rows.map((event) => event.value).filter((eventData) => eventData._id !== editEventId)

                const isConflict = updateEvent.some((event) => {
                    const eventData = event.data;

                    if (eventData.artist === artist && eventData.date === date) {
                        // Existing event time
                        const [existEventHr, existEventMin] = eventData.time.split(':').map(Number);
                        const existEventTime = new Date();
                        existEventTime.setHours(existEventHr, existEventMin, 0, 0);
